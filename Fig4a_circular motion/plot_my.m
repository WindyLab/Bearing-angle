...
y_max = max([reserve_state_tar(2, :), reserve_state_obs(2, :), ...
            reserve_est_state_ba(2, :), reserve_est_state_bo(2, :)])+4;
...
axis(ax_dis, [0, (total_t)*dt, 0, y_max*1.02]);
...
h_ness_bo = plot(ax_nees, (1:total_t)*dt, reserve_nees_bo(1, 1:total_t),  'color', color_est_t_bo, 'LineWidth',1.5);
hold on
h_ness_ba = plot(ax_nees, (1:total_t)*dt, reserve_nees_ba(1, 1:total_t),  'color', color_est_t_ba, 'LineWidth',1.5);
hold on

legend([h_ness_bo, h_ness_ba, p], {'bearing-only', 'bearing-angle', '99% confidence interval'}, 'FontSize', fontsize);
axis(ax_nees, [0, (total_t)*dt, 0, 25]);
xlabel('time (s)','FontSize', title_fontsize);
...
axis(ax_l, [0, (total_t)*dt, 0.8, y_max*1.1]);
...
for t = 1:n:(total_t-1)
    ...
    handle_t.Position = [reserve_state_tar(1,1)-target_size/2, reserve_state_tar(2,1)-target_size/2,target_size,target_size];
    % estimated position
