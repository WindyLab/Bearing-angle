%% Numerical simulation for Fig 4c
% observer: guidance
% target: circul-shape, constant velocity
% 
clear all
clc
close all
...
total_time = 3.5;              % total simulation time for each Monte Carlo iteration
...
est_init_tar_size = 1.6;       % initial estimated target's size
% motion states (pos, vel)
reserve_state_obs = zeros(4, total_t);
reserve_state_tar = zeros(4, total_t);
reserve_state_obs(:, 1) = [0;0;0;0];
reserve_state_tar(:, 1) = [0;10;3;1];
...
for t = 1:total_t
    if t < total_t
        reserve_state_tar(3:4, t+1) = reserve_state_tar(3:4, t);
        reserve_state_tar(1:2, t+1) = reserve_state_tar(1:2, t) + dt * reserve_state_tar(3:4, t);
        reserve_state_obs(:, t+1) = control_observer_3_guidance(v_o, ...
                                                 reserve_state_tar(:, t), reserve_state_obs(:, t), dt);
    end
end
% Mote Carlo simulation
for n = 1:total_N
    P_bo = 0.1*diag([1, 1, 1, 1]);
    P_ba = 0.1*diag([1, 1, 1, 1, 1]);
    for t = 1:total_t
        ...
        if t == 1
            reserve_est_state_ba(:,t, n)=[[0; 13]; [0;0]; est_init_tar_size];
            reserve_est_state_bo(:,t, n) = [[0; 13]; [0;0]];
